...
export const useTabStore = create<TabStoreState>((set, get) => ({
    ...
    initializeRpc: async () => {
        ...
        await hyaenidae.rpc.ask("shell:ready");
    },
    createTab: async () => {
        await hyaenidae.rpc.ask("shell:tab-new");
    },
    focusTabRpc: async (id) => {
        await hyaenidae.rpc.ask("shell:tab-focus", { id });
    },
    ...
    goBack: async () => {
        ...
        await hyaenidae.rpc.ask("shell:tab-go-back", { id: activeTabId });
    },
    goForward: async () => {
        const { activeTabId } = get();
        if (activeTabId === null) {
        ...
        await hyaenidae.rpc.ask("shell:tab-go-forward", { id: activeTabId });
    },
    refreshOrStop: async () => {
    ...
    toggleAgentPanel: async () => {
        await hyaenidae.rpc.ask("shell:toggle-agent-panel", {
            isAgentPanelOpen: !get().isAgentPanelOpen,
        });
