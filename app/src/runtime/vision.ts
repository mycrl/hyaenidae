        ...
        return candidates
            .map((element) => {
                ...
                const score = tokens.reduce(
                    (total, token) =>
                        total + (haystack.includes(token) ? 1 : 0),
                    0,
                );
            ...
            .filter(
                (candidate) =>
                    candidate.score > 0 && candidate.element.selector,
            )
            ...
            .map((candidate) => ({
                point: {
                    x:
                        candidate.element.bounds.x +
                        candidate.element.bounds.width / 2,
                    y:
                        candidate.element.bounds.y +
                        candidate.element.bounds.height / 2,
                },
                ...
                confidence:
                    ...
                    Math.max(2, Math.ceil(tokens.length * 0.6))
                        ...
                        : candidate.score >= 2
                          ? "medium"
                          : "low",
                ...(candidate.element.role === undefined
                    ? {}
                    : { role: candidate.element.role }),
                ...((candidate.element.text ?? candidate.element.ariaLabel) ===
                undefined
                    ? {}
                    : {
                          text:
                ...
                ...(candidate.element.href === undefined
                    ? {}
                    : { url: candidate.element.href }),
            }));
    }
}
