...
import { createBrowserTools, type VisionInspector } from "./tools";
    ...
    async ask(options: ModelAskOptions): Promise<AgentRunStream> {
        ...
        const visionInspector: VisionInspector = {
            inspect: async ({ model, prompt, tabId }) => {
                if (!options.browser) {
                    throw new Error("Browser runtime is unavailable for vision inspection.");
                }
        ...
        const result = await run(agent, options.message, {
            stream: true,
        ...
        const liveStream = new AgentRunStream(result, options.locale);
